...
# ------------------------------
# NUEVO: Forzar reserva para mañana
# ------------------------------
...
def obtener_hora_reserva_forzada():
    # Usa la hora del día real que tocaría mañana
...
# ------------------------------
# SELECTOR ROBUSTO CON REINTENTOS
# ------------------------------
def click_con_reintento(page, selector, intentos=3):
    for i in range(intentos):
        try:
            page.get_by_text(selector, exact=False).click(timeout=8000)
            return
        except Exception:
            if i == intentos - 1:
                raise
...
def reservar(forzar_manana=False):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            print("Abriendo menú…")
            page.goto(URL_MENU, wait_until="domcontentloaded")

            click_con_reintento(page, "Reserva")
            ...
            click_con_reintento(page, "Confirmar")
...
if __name__ == "__main__":
    for i in range(5):
        try:
            ...
            reservar(forzar_manana=True)   # <--- NUEVO: fuerza reserva mañana
            break
